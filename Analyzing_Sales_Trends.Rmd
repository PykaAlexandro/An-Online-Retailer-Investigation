...
To investigate further, we can at first show the distribution of values, to assess the magnitude of variations.
...
More interesting is that it didn't return a seasonality, meaning that the retailer has no significant recurring highs or lows in sales over fixed chronological segments (like weeks or months).
...
Instead of the overall sales, we can direct this kind of analysis also to a singular stock code.   
We can take the most popular one, `85123A`, ascertained in a [previous document](https://pykaalexandro.github.io/An-Online-Retailer-Investigation/Investigating_Items.html#-_most_popular_items).

We can start with its distribution of quantities per day,
...
The number of stock codes never bought during those days is much larger (more than `1000`).
