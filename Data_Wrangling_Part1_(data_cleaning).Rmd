...
df %>% 
  ...
  filter(any(StockCode == "D")) %>%
  ungroup() %>%
  count(Invoice, name = "Number of Items") %>%
...
and no invoices with both positive and negative values of `Quantity`.
...
- A `Description` column with several values (`NAs`, typos, updated descriptions or notes) for the same stock code, the same description assigned to different stock codes  
